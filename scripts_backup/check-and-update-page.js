// page-1764459561986ページのコンテンツを確認して更新するスクリプト
// ブラウザのコンソールで実行してください
...
(async function() {
  ...
  const targetPageId = 'page-1764459561986';

  // 「1. 出産支援パーソナルアプリケーションとは」のコンテンツ（HTML形式）
  const maternitySupportContent = `<div style="margin-bottom: 40px">
  ...
  try {
    ...
    console.log('現在のページ一覧:', pages.map(p => ({ id: p.id, title: p.title, contentLength: p.content?.length || 0 })));
    ...
    if (targetPageIndex === -1) {
      console.error(`ページ ${targetPageId} が見つかりません。利用可能なページ:`, pages.map(p => `${p.id} (${p.title})`));
      return;
    }

    console.log(`ページ ${targetPageId} が見つかりました:`, pages[targetPageIndex]);
    console.log('現在のコンテンツ:', pages[targetPageIndex].content);

    // ページのコンテンツを更新
    const updatedPages = [...pages];
    updatedPages[targetPageIndex] = {
      ...updatedPages[targetPageIndex],
      content: maternitySupportContent.trim(),
    };

    console.log('更新後のコンテンツ（最初の100文字）:', updatedPages[targetPageIndex].content.substring(0, 100));
    ...
    console.log('ページをリロードしてください（F5またはCmd+R）');
    
    // 確認のため、再度読み込んで表示
    const verifySnapshot = await getDocs(conceptsQuery);
    const verifyDoc = verifySnapshot.docs[0];
    const verifyData = verifyDoc.data();
    const verifyPages = verifyData.pages || [];
    const verifyPage = verifyPages.find(p => p.id === targetPageId);
    if (verifyPage) {
      console.log('✅ 保存確認: コンテンツの長さ:', verifyPage.content?.length || 0);
    }
  } catch (error) {
