/**
 * test3ページのコンテンツを「1. 出産支援パーソナルアプリケーションとは」の内容に更新するスクリプト
 * 
 * 使用方法:
 * 1. Firebase Admin SDKを設定
 * 2. このスクリプトを実行
 */
...
async function updateTest3Content() {
  try {
    // ユーザーIDとサービスID、コンセプトIDを指定
    const userId = 'YOUR_USER_ID'; // 実際のユーザーIDに置き換える
    ...
    const test3PageIndex = pages.findIndex(
      (page: any) => page.title?.toLowerCase() === 'test3'
    );

    if (test3PageIndex === -1) {
      console.error('test3ページが見つかりません');
      return;
    }
