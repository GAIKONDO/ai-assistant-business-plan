...
export default function ReferencesPage() {
  ...
  useEffect(() => {
    const loadReferences = async () => {
      ...
      try {
        const docRef = doc(db, 'companyBusinessPlan', planId);
        ...
        if (docSnap.exists()) {
          ...
          if (savedReferences.length === 0) {
            ...
            // Firestoreに保存
            await setDoc(docRef, { references: defaultReferences }, { merge: true });
          } else {
            ...
            if (missingReferences.length > 0) {
              ...
              // Firestoreに保存
              await setDoc(docRef, { references: updatedReferences }, { merge: true });
            } else {
              setReferences(savedReferences);
            }
          }
        } else {
          // ドキュメントが存在しない場合、デフォルトの参考文献を追加
          setReferences(defaultReferences);
          // Firestoreに保存
          await setDoc(docRef, { references: defaultReferences }, { merge: true });
        }
