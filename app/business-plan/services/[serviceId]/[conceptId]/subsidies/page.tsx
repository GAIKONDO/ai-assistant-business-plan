...
export default function SubsidiesPage() {
  ...
  const getCurrentSubMenu = () => {
    ...
    return SUB_MENU_ITEMS.find(item => item.path === lastSegment)?.id || 'subsidies';
  };
      ...
      <div style={{ display: 'flex', gap: '32px' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ marginBottom: '24px' }}>
            ...
            <p style={{ margin: 0, fontSize: '14px', color: 'var(--color-text-light)' }}>補助金・助成金</p>
          </div>
          <div className="card"><p style={{ color: 'var(--color-text-light)', fontSize: '14px' }}>補助金・助成金の内容をここに表示します</p></div>
        </div>
      </div>
    </Layout>
  );
