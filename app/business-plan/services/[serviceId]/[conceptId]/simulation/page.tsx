...
export default function SimulationPage() {
  ...
  const getCurrentSubMenu = () => {
    ...
    return SUB_MENU_ITEMS.find(item => item.path === lastSegment)?.id || 'simulation';
  };
      ...
      <div style={{ display: 'flex', gap: '32px' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ marginBottom: '24px' }}>
            ...
            <p style={{ margin: 0, fontSize: '14px', color: 'var(--color-text-light)' }}>シミュレーション</p>
          </div>
          <div className="card"><p style={{ color: 'var(--color-text-light)', fontSize: '14px' }}>シミュレーションの内容をここに表示します</p></div>
        </div>
      </div>
    </Layout>
  );
