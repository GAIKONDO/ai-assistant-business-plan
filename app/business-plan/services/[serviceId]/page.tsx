...
export default function ServiceDetailPage() {
  ...
  const loadConcepts = useCallback(async () => {
    ...
    try {
      ...
      for (const fixedConcept of fixedConcepts) {
      ...
      console.log('loadConcepts: 構想データ取得完了', {
        ...
        fixedConceptDates: fixedDatesMap,
      });
  ...
  const handleToggleFavorite = async (conceptId: string, currentFavorite: boolean) => {
    if (!db) return;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '20px', marginTop: showConceptForm ? '24px' : '0' }}>
          ...
          {concepts.filter((concept) => {
            ...
            if (conceptFilter === 'favorite') {
              return (concept as any).isFavorite === true;
            }
            const pagesBySubMenu = concept.pagesBySubMenu;
            const isComponentized = pagesBySubMenu && 
              typeof pagesBySubMenu === 'object' && 
            ...
            <div style={{
              ...
              <div>
                ...
                <p style={{
                  ...
                  {fixedConcepts.length + concepts.length}件の構想
                </p>
              </div>
              <button
                onClick={() => {
