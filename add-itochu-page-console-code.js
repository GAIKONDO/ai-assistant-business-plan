...
// ページ: http://localhost:3005/business-plan/company/5fMIys3S9yCQNCtEpIDH/overview

(async function() {
  try {
    // Firebaseのインポート（既にページで読み込まれている場合）
    // グローバルスコープから取得を試みる
    let db, doc, getDoc, setDoc, serverTimestamp;
    
    // 既存のFirebaseインスタンスを取得
    ...
    if (!db) {
      // 最後の手段：グローバルスコープから直接取得
      // このコードは、既にFirebaseが初期化されているページで実行する必要があります
      console.error('Firebaseが初期化されていません。ページをリロードしてから再度実行してください。');
      return;
    }
    
    const planId = '5fMIys3S9yCQNCtEpIDH';
    const subMenuId = 'overview';
    const newPageId = `page-${Date.now()}`; // 新しいページIDを生成

    // 伊藤忠商事の「三方良し」についてのコンテンツ
    const itochuContent = `<div class="page-section-content">
  ...
  <div style="margin-top: 32px; margin-bottom: 32px; padding-left: 11px;">
    ...
    if (!planDoc.exists()) {
      throw new Error('事業計画が見つかりませんでした。');
    }
    ...
    // 新しいページを作成（3ページ目として追加）
    const newPage = {
      id: newPageId,
      pageNumber: currentSubMenuPages.length, // ページ番号は自動的に設定
      title: '伊藤忠商事の三方良しについて',
    ...
    console.log('ページ番号:', newPage.pageNumber);
