...
export default function ForceDirectedGraph3D({
  ...
  // Firebaseからデータを取得（React Queryでキャッシュ）
  const { data: graphData, isLoading: queryLoading } = useQuery({
    queryKey: ['forceDirectedGraph3D', auth?.currentUser?.uid],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      nodes.forEach((node) => {
        ...
        text.color = '#1a1a1a';
        text.fontWeight = node.type === 'company' ? '600' : node.type === 'project' ? '500' : '400';
        text.anchorX = 'center';
        text.anchorY = 'middle';
