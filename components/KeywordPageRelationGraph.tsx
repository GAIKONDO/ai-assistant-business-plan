...
export default function KeywordPageRelationGraph({
  ...
  const getPageInfo = async (pageId: string): Promise<{
    ...
    if (!db || !auth?.currentUser) return { page: null };

    try {
      const userId = auth.currentUser.uid;

      // 会社事業計画から検索
      const companyPlansSnapshot = await getDocs(
        query(collection(db, 'companyBusinessPlan'), where('userId', '==', userId))
      );

      for (const planDoc of companyPlansSnapshot.docs) {
      ...
      const conceptsSnapshot = await getDocs(
        query(collection(db, 'concepts'), where('userId', '==', userId))
      );
  ...
  const handleSearch = async () => {
    if (!keyword.trim() || !db || !auth?.currentUser) {
